...
if [[ -f "$MPD_CONF" ]]; then
    # Check if the FIFO output is already configured
    if grep -q "/tmp/mpd.fifo" "$MPD_CONF" 2>/dev/null; then
        skip "mpd.conf already has FIFO output configured"
        WRITE_CONF=false
    else
        warn "mpd.conf exists but has no FIFO output (required for the visualizer)."
...
# ── Optional: HTTP stream (for Web Audio fallback in Settings → Visualizer) ──
# audio_output {
#     type            "httpd"
